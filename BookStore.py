...
class BookStore:
    ...
    def load_catalog(self, fileName: str, catalog_ds: str, graph_ds: str, sort_by: str):
        ...
        try:
            ...
            # loading graph
            if graph_ds == '1':
                # FIXME: Initialize to an AdjacencyList
                self.similarityGraph = AdjacencyList.AdjacencyList(n)
            elif graph_ds == '2':
                # FIXME: Initialize to an AdjacencyList
                self.similarityGraph = AdjacencyMatrix.AdjacencyMatrix(n)
            else:
            ...
            for source_node in range(self.bookCatalog.size()):
                ...
                for j in range(len(similar)):
                    ...
                    # FIXME: Get the index corresponding to the key
                    target_node = self.mapKeysToIdxs.find(key)
    ...
    def _display_similar(self, idx: int):
        ...
        try:
            ...
            reachable_nodes = self.similarityGraph.bfs(idx)
            for i in range(reachable_nodes.size()):
                if i == 0 or self.similarityGraph.has_edge(idx, reachable_nodes.get(i)):
                    similar.append(reachable_nodes.get(i))
