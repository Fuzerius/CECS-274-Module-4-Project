        ...
        if not by_rank:
            ...
            try:
                ...
                for i in range(self.bookCatalog.size()):
                    book = self.bookCatalog.get(i)
                    if infix.lower() in book.title.lower():
                        # Create a new Book object with rank as -1 * original rank
                        ranked_book = Book(book.key, book.title, book.group, -1 * int(book.rank), book.similar)
                ...
                for i in range(self.booksByRank.size()):
                    ...
                    top_book.rank = abs(top_book.rank)  # FIXME: Replace 0 with the absolute value of the rank of the top-ranking book
                    print("-" * 25 + f"\nTop Match #{printed + 1}:\n{top_book}\n")
                    printed += 1
                    if printed == n:
                        break
